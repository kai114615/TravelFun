...
const SYSTEM_PROMPT = `你是旅遊趣網站的專業旅遊顧問。請使用繁體中文回覆。回答範圍包括：全臺景點與活動介紹、交通建議、美食推薦、住宿選擇和行程規劃。回答要詳細實用，並友善專業。`;
...
async function tryBackupApiFormat(userMessage: string): Promise<string | null> {
  try {
    ...
    console.error('備用 API 格式也失敗:', error);
    return null;
  }
}
...
<template>
  <div class="fixed bottom-4 right-4 z-50">
    <!-- 聊天按钮 -->
    <NButton v-if="!isChatOpen" circle type="primary" size="large" class="shadow-lg animate-bounce" @click="toggleChat">
      <template #icon>
        <NIcon>
          <ChatOutlined />
        </NIcon>
    ...
    <NCard v-else class="w-[350px] shadow-lg rounded-lg chat-window">
      <div class="flex flex-col h-[500px]">
        ...
        <!-- API 測試結果 -->
        <div v-if="apiTestResult" class="mb-3 p-2 text-sm border rounded-lg"
          :class="{ 'bg-green-50 border-green-200': apiTestResult.includes('✅'), 'bg-red-50 border-red-200': apiTestResult.includes('❌'), 'bg-yellow-50 border-yellow-200': apiTestResult.includes('⚠️'), 'bg-blue-50 border-blue-200': apiTestResult.includes('正在測試') }">
          ...
          <div class="space-y-3">
            <template v-for="(msg, index) in messages" :key="index">
              <div class="flex" :class="msg.role === 'user' ? 'justify-end' : 'justify-start'">
                <div class="max-w-[85%] rounded-2xl p-2 px-3" :class="[
                  msg.role === 'user'
        ...
        <!-- 输入区域 -->
        <div class="flex gap-2">
          <NInput v-model:value="inputMessage" type="textarea" :rows="1" placeholder="請輸入您的問題..."
            @keypress="handleKeyPress" />
          <NButton circle type="primary" :disabled="isLoading" @click="sendMessage">
            <template #icon>
              <NIcon>
                <SendOutlined />
              </NIcon>
