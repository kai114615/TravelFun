swfobject.addDomLoadEvent(() => {
  ...
  const sourcePic1Url = $.Cookie('swf1')
  let sourcePic2Url = $.Cookie('swf2');
  ...
  const callback = function (json) {
    ...
    switch (json.code) {
      ...
      case 5:
        ...
        if (json.type == 0) {
          const e = this;
          const html = $('<div class="imgList"/>');
          for (let i = 0; i < json.content.avatarUrls.length; i++)
          {
          ...
          {
            button.push({ text: '修改头像', callback() {
              this.close();
  ...
  // 点击取消按钮的事件
  $('.button_cancel').click(() => {
    const activedTab = $('#avatar-tab li.active')[0].id
  ...
  // ------------------------------------------------------------------------------示例二
  const swf2 = new fullAvatarEditor('swf2', {
    id: 'swf2',
