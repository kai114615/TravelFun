...
;!(function (win, undefined) {
  const config = {
    ...
    chating: {},
    hosts: (function () {
      let dk = location.href.match(/:\d+/);
      dk = dk ? dk[0] : '';
      return `http://${document.domain}${dk}/`
    })(),
    json(url, data, callback, error) {
      return $.ajax({
        type: 'POST',
        url,
        data,
  ...
  }; const dom = [$(window), $(document), $('html'), $('body')]; const xxim = {};

  // 主界面tab
  xxim.tabs = function (index) {
    const node = xxim.node;
  ...
  xxim.expend = function () {
    ...
    if (xxim.layimNode.attr('state') !== '1') {
      ...
      xxim.layimNode.stop().animate({ right: 1 }, config.aniTime, () => {
  ...
  xxim.popchat = function (param) {
    const node = xxim.node; const log = {};
    ...
    log.success = function (layero) {
      ...
      // 最小化聊天窗
      xxim.chatbox.find('.layer_setmin').on('click', () => {
        const indexs = layero.attr('times');
        layero.hide()
      ...
      log.chatlist.on('click', 'li em', function (e) {
        const parents = $(this).parent(); const dataType = parents.attr('type');
        const dataId = parents.attr('data-id'); const index = parents.index();
        const chatlist = log.chatlist.find('li'); let indexs
  ...
  xxim.tabchat = function (param) {
    const node = xxim.node; const log = {}; const keys = param.type + param.id;
    xxim.nowchat = param
  ...
  xxim.getGroups = function (param) {
    const keys = param.type + param.id; let str = '';
    const groupss = xxim.chatbox.find(`#layim_group${keys}`)
    groupss.addClass('loading')
    config.json(config.api.groups, {}, (datas) => {
      if (datas.status === 1) {
        let ii = 0; const lens = datas.data.length;
        if (lens > 0) {
          for (; ii < lens; ii++) {
  ...
  xxim.transmit = function () {
    ...
    log.send = function () {
      ...
      if (data.content.replace(/\s/g, '') === '') {
        ...
        // 此处皆为模拟
        const keys = xxim.nowchat.type + xxim.nowchat.id;
  ...
  xxim.event = function () {
    ...
    node.tabs.on('click', function () {
      const othis = $(this); const index = othis.index();
      xxim.tabs(index)
    });

    // 列表展收
    node.list.on('click', 'h5', function () {
      const othis = $(this); const chat = othis.siblings('.xxim_chatlist'); const parentss = othis.find('i')
  ...
  xxim.getDates = function (index) {
    const api = [config.api.friend, config.api.group, config.api.chatlog]
    const node = xxim.node; const myf = node.list.eq(index);
    myf.addClass('loading')
    config.json(api[index], {}, (datas) => {
      if (datas.status === 1) {
        let i = 0; const myflen = datas.data.length; let str = ''; let item
        if (myflen > 1) {
          if (index !== 2) {
            for (; i < myflen; i++) {
  ...
  // 渲染骨架
  xxim.view = (function () {
    const xximNode = xxim.layimNode = $('<div id="xximmm" class="xxim_main">'
