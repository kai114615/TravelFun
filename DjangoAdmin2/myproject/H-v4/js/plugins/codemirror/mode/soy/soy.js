...
(function (mod) {
  ...
  const indentingTags = ['template', 'literal', 'msg', 'fallbackmsg', 'let', 'if', 'elseif', 'else', 'switch', 'case', 'default', 'foreach', 'ifempty', 'for', 'call', 'param', 'deltemplate', 'delcall', 'log'];
  ...
  CodeMirror.defineMode('soy', (config) => {
    ...
    return {
        ...
        switch (last(state.soyState)) {
          ...
          case 'tag':
            if (stream.match(/^\/?\}/)) {
              ...
              else state.indent -= (stream.current() == '/}' || !indentingTags.includes(state.tag) ? 2 : 1) * config.indentUnit
              state.soyState.pop()
              return 'keyword';
      ...
      indent (state, textAfter) {
        let indent = state.indent; const top = last(state.soyState);
