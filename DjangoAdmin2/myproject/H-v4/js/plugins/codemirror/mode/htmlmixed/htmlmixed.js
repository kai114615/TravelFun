...
(function (mod) {
  ...
  CodeMirror.defineMode('htmlmixed', (config, parserConfig) => {
    const htmlMode = CodeMirror.getMode(config, { name: 'xml', htmlMode: true, multilineTagIndentFactor: parserConfig.multilineTagIndentFactor, multilineTagIndentPastTag: parserConfig.multilineTagIndentPastTag })
    const cssMode = CodeMirror.getMode(config, 'css');

    const scriptTypes = []; const scriptTypesConf = parserConfig && parserConfig.scriptTypes
    ...
    function maybeBackup(stream, pat, style) {
      ...
      const close = cur.search(pat); let m;
