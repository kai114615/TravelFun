...
(function (mod) {
  ...
  CodeMirror.defineMode('r', (config) => {
    function wordObj(str) {
      const words = str.split(' '); const res = {}
    ...
    function push(state, type, stream) {
      state.ctx = { type, indent: state.indent, align: null, column: stream.column(), prev: state.ctx }
    }
    function pop(state) {
    ...
    return {
      startState () {
        return { tokenize: tokenBase, ctx: { type: 'top', indent: -config.indentUnit, align: false }, indent: 0, afterIdent: false };
      },
      ...
      indent (state, textAfter) {
        ...
        const firstChar = textAfter && textAfter.charAt(0); const ctx = state.ctx
