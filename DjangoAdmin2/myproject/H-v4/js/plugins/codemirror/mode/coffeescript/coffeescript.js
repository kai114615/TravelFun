...
(function (mod) {
  ...
  CodeMirror.defineMode('coffeescript', (conf, parserConf) => {
    ...
    const delimiters = /^(?:[()[\]{},:`=;]|\.{1,3})/
    const identifiers = /^[_A-Z$][\w$]*/i;
    const properties = /^(@|this\.)[_A-Za-z$][\w$]*/

    const wordOperators = wordRegexp(['and', 'or', 'not', 'is', 'isnt', 'in', 'instanceof', 'typeof'])
    let indentKeywords = ['for', 'while', 'loop', 'if', 'unless', 'else', 'switch', 'try', 'catch', 'finally', 'class']
    const commonKeywords = ['break', 'by', 'continue', 'debugger', 'delete', 'do', 'in', 'of', 'new', 'return', 'then', 'this', '@', 'throw', 'when', 'until', 'extends'];
    ...
    function indent(stream, state, type) {
      type = type || 'coffee'
      let offset = 0; let align = false; let alignOffset = null;
    ...
    const external = {
      ...
      indent(state, text) {
        ...
        const closer = text && '])}'.includes(text.charAt(0));
