...
(function (mod) {
  ...
  const mac_geMountainLion = /Mac OS X 1\d\D([89]|\d\d)\D/.test(navigator.userAgent)
  ...
  function keyMapChanged(cm) {
    const map = keyMap[cm.options.keyMap]; const style = map.style
  ...
  function updateScrollbars(cm, measure) {
    ...
    const d = cm.display; const sWidth = scrollbarWidth(d.measure)
  ...
  function visibleLines(display, doc, viewport) {
    ...
    const from = lineAtHeight(doc, top); const to = lineAtHeight(doc, bottom)
  ...
  function postUpdateDisplay(cm, update) {
    let force = update.force; let viewport = update.viewport;
    ...
    if (cm.options.lineNumbers || markers) {
      ...
      if (cm.options.lineNumbers && (!markers || !markers['CodeMirror-linenumbers'])) {
        lineView.lineNumber = gutterWrap.appendChild(
          elt('div', lineNumberFor(cm.options, lineN), 'CodeMirror-linenumber CodeMirror-gutter-elt', `left: ${dims.gutterLeft['CodeMirror-linenumbers']}px; width: ${
  ...
  function skipAtomic(doc, pos, bias, mayClear) {
    let flipped = false; let curPos = pos;
    let dir = bias || 1;
    doc.cantEdit = false;
    search: for (;;) {
  ...
  function drawSelection(cm) {
    const display = cm.display; const doc = cm.doc; const result = {}
  ...
  function drawSelectionRange(cm, range, output) {
    ...
    const padding = paddingH(cm.display); const leftSide = padding.left; const rightSide = display.lineSpace.offsetWidth - padding.right
  ...
  function paddingH(display) {
    ...
    const data = { left: Number.parseInt(style.paddingLeft), right: Number.parseInt(style.paddingRight) };
  ...
  function measureCharInner(cm, prepared, ch, bias) {
    ...
    const result = { left: (collapse == 'right' ? rect.right : rect.left) - prepared.rect.left, right: (collapse == 'left' ? rect.left : rect.right) - prepared.rect.left, top, bottom: bot };
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    for (;;) {
      if (bidi ? to == from || to == moveVisually(lineObj, from, 1) : to - from <= 1) {
        ...
        const pos = PosWithInfo(lineNo, ch, ch == from ? fromOutside : toOutside, xDiff < -1 ? -1 : xDiff > 1 ? 1 : 0)
        return pos;
      }
  ...
  function endOperation_finish(op) {
    ...
    if (op.scrollToPos) {
      const coords = scrollPosIntoView(cm, clipPos(doc, op.scrollToPos.from), clipPos(doc, op.scrollToPos.to), op.scrollToPos.margin)
  ...
  function readInput(cm) {
    const input = cm.display.input; let prevInput = cm.display.prevInput; const doc = cm.doc
    ...
    const inserted = text.slice(same); const textLines = splitLines(inserted)
    ...
    for (let i = doc.sel.ranges.length - 1; i >= 0; i--) {
      ...
      const changeEvent = { from, to, text: multiPaste ? multiPaste[i % multiPaste.length] : textLines, origin: cm.state.pasteIncoming ? 'paste' : cm.state.cutIncoming ? 'cut' : '+input' }
  ...
  function resetInput(cm, typing) {
    let minimal; let selected; const doc = cm.doc
  ...
  function registerEventHandlers(cm) {
    ...
    on(d.input, 'paste', () => {
      ...
      if (webkit && !cm.state.fakedLastChar && !(new Date() - cm.state.lastMiddleDown < 200)) {
        const start = d.input.selectionStart; const end = d.input.selectionEnd
        d.input.value += '$'
  ...
  function leftButtonDown(cm, e, start) {
    ...
    const sel = cm.doc.sel; const modifier = mac ? e.metaKey : e.ctrlKey
  ...
  function leftButtonSelect(cm, e, start, type, addNew) {
    ...
    let ourRange; let ourIndex; let startSel = doc.sel;
  ...
  function gutterEvent(cm, e, type, prevent, signalfn) {
    try { var mX = e.clientX; var mY = e.clientY; }
  ...
  function handleKeyBinding(cm, e) {
    ...
    const startMap = getKeyMap(cm.options.keyMap); const next = startMap.auto
    ...
    const name = keyName(e, true); let handled = false;
    ...
    if (e.shiftKey) {
      ...
      handled = lookupKey(`Shift-${name}`, keymaps, (b) => { return doHandleBinding(cm, b, true) })
        || lookupKey(name, keymaps, (b) => {
          if (typeof b == 'string' ? /^go[A-Z]/.test(b) : b.motion)
            return doHandleBinding(cm, b);
        })
    } else {
      handled = lookupKey(name, keymaps, (b) => { return doHandleBinding(cm, b); })
    }
  ...
  function handleCharBinding(cm, e, ch) {
    const handled = lookupKey(`'${ch}'`, allKeyMaps(cm), (b) => { return doHandleBinding(cm, b, true) });
    if (handled) {
      e_preventDefault(e);
      restartBlink(cm);
  ...
  function scrollPosIntoView(cm, pos, end, margin) {
    ...
    for (let limit = 0; limit < 5; limit++) {
      let changed = false; const coords = cursorCoords(cm, pos)
      const endCoords = !end || end == pos ? coords : cursorCoords(cm, end)
      const scrollPos = calculateScrollPos(cm, Math.min(coords.left, endCoords.left), Math.min(coords.top, endCoords.top) - margin, Math.max(coords.left, endCoords.left), Math.max(coords.bottom, endCoords.bottom) + margin)
  ...
  function calculateScrollPos(cm, x1, y1, x2, y2) {
    ...
    const screen = display.scroller.clientHeight - scrollerCutOff; const result = {}
  ...
  function resolveScrollToPos(cm) {
    ...
    if (range) {
      ...
      const sPos = calculateScrollPos(cm, Math.min(from.left, to.left), Math.min(from.top, to.top) - range.margin, Math.max(from.right, to.right), Math.max(from.bottom, to.bottom) + range.margin)
  ...
  CodeMirror.prototype = {
    ...
    getTokenAt (pos, precise) {
      const doc = this.doc;
      pos = clipPos(doc, pos)
      const state = getStateBefore(this, pos.line, precise); const mode = this.doc.mode;
      ...
      return { start: stream.start, end: stream.pos, string: stream.current(), type: style || null, state };
    },
    ...
    heightAtLine (line, mode) {
      let end = false; const last = this.doc.first + this.doc.size - 1;
    ...
    getScrollInfo () {
      const scroller = this.display.scroller; const co = scrollerCutOff;
    ...
    scrollIntoView: methodOp(function (range, margin) {
      ...
      if (range.from.line != null) {
        ...
        const sPos = calculateScrollPos(this, Math.min(range.from.left, range.to.left), Math.min(range.from.top, range.to.top) - range.margin, Math.max(range.from.right, range.to.right), Math.max(range.from.bottom, range.to.bottom) + range.margin)
  ...
  option('lineNumberFormatter', (integer) => { return integer }, guttersChanged, true);
  ...
  var commands = CodeMirror.commands = {
    ...
    goLineStart (cm) {
      cm.extendSelectionsBy((range) => { return lineStart(cm, range.head.line) }, { origin: '+move', bias: 1 })
    },
    goLineStartSmart (cm) {
      cm.extendSelectionsBy((range) => {
    ...
    goLineEnd (cm) {
      cm.extendSelectionsBy((range) => { return lineEnd(cm, range.head.line) }, { origin: '+move', bias: -1 })
    },
    goLineRight (cm) {
      cm.extendSelectionsBy((range) => {
        const top = cm.charCoords(range.head, 'div').top + 5;
  ...
  function markText(doc, from, to, options, type) {
    ...
    if (marker.addToHistory)
      addChangeToHistory(doc, { from, to, origin: 'markText' }, doc.sel, Number.NaN)
  ...
  LineWidget.prototype.clear = function () {
    const cm = this.cm; const ws = this.line.widgets; const line = this.line; const no = lineNo(line)
  ...
  LineWidget.prototype.changed = function () {
    const oldH = this.height; const cm = this.cm; const line = this.line
    this.height = null;
  ...
  function buildLineContent(cm, lineView) {
    ...
    const builder = { pre: elt('pre', [content]), content, col: 0, pos: 0, cm }
    lineView.measure = {};
  ...
  function buildToken(builder, text, style, startStyle, endStyle, title) {
    if (!text) return;
    const special = builder.cm.options.specialChars; let mustWrap = false;
  ...
  function insertLineContent(line, builder, styles) {
    ...
    const len = allText.length; let pos = 0; var i = 1; let text = ''; let style;
  ...
  function updateDoc(doc, change, markedSpans, estimateHeight) {
    ...
    const from = change.from; const to = change.to; const text = change.text
  ...
  function isWordChar(ch, helper) {
    ...
    if (helper.source.includes('\\w') && isWordCharBasic(ch)) return true;
