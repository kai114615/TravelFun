 ...
 */
  const Markdown = expose.Markdown = function Markdown (dialect) {
  ...
  expose.toHTML = function toHTML(source, dialect, options) {
    const input = expose.toHTMLTree(source, dialect, options);
  ...
  const mk_block = Markdown.mk_block = function (block, trail, line) {
  ...
  Markdown.prototype.toTree = function toTree(source, custom_root) {
    const blocks = Array.isArray(source) ? source : this.split_blocks(source);
  ...
  Markdown.dialects.Gruber = {
    block: {
      atxHeader: function atxHeader(block, next) {
        ...
        const header = ['header', { level: m[1].length }];
      ...
      // There might also be adjacent code block to merge.
        ...
        // Now pull out the rest of the lines
          const b = this.loop_re_over_block(
            re,
            block.valueOf(),
            (m) => { ret.push(m[1]); }
          )
      ...
      // Use a closure to hide a few variables.
        ...
        function add(li, loose, inline, nl) {
          ...
          const add_to = Array.isArray(li[li.length - 1]) && li[li.length - 1][0] == 'para'
            ? li[li.length - 1]
            : li
        ...
        function paragraphify(s, i, stack) {
          ...
          if (Array.isArray(last_li[1]) && last_li[1][0] == 'para') {
            return
          }
          if (i + 1 == stack.length) {
          // Last stack frame
        ...
        return function (block, next) {
          ...
          function make_list(m) {
            const list = bullet_list.exec(m[2])
              ? ['bulletlist']
              : ['numberlist'];
          ...
          // Split into lines preserving new lines at end of line
            ...
            if (next_block.match(is_list_re) || next_block.match(/^ /)) {
              ...
              const hr = this.dialect.block.horizRule(block, next);
      ...
      referenceDefn: function referenceDefn(block, next) {
        ...
        const b = this.loop_re_over_block(re, block, (m) => {
  ...
  Markdown.dialects.Gruber.inline = {
    ...
    '__oneElement__': function oneElement(text, patterns_or_re, previous_nodes) {
      ...
      const re = new RegExp(`([\\s\\S]*?)(${patterns_or_re.source || patterns_or_re})`);
    ...
    '[': function link(text) {
      ...
      const res = Markdown.DialectHelpers.inline_until_char.call(this, text.substr(1), ']');
      ...
      if (m) {
        ...
        if (!m[3]) {
          let open_parens = 1; // One open that isn't in the capture
  ...
  Markdown.DialectHelpers.inline_until_char = function (text, want) {
    ...
    while (true) {
      ...
      const res = this.dialect.inline.__oneElement__.call(this, text.substr(consumed));
      consumed += res[0]
