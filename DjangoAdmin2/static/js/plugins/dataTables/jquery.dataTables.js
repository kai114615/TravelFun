...
(/** @lends <global> */function (window, document, $, undefined) {
  ...
  (/** @lends <global> */($) => {
    ...
    const _intVal = function (s) {
      const integer = Number.parseInt(s, 10)
      return !isNaN(integer) && isFinite(s) ? integer : null
    }

    const _isNumber = function (d, formatted) {
      if (formatted && typeof d === 'string') {
        d = d.replace(_re_formatted_numeric, '');
      }

      return !d || d === '-' || (!isNaN(Number.parseFloat(d)) && isFinite(d))
    }
    ...
    const _htmlNumeric = function (d, formatted) {
      if (_empty(d)) {
        return true
      }
    ...
    function _fnHungarianMap (o)
    {
      ...
      $.each(o, (key, val) => {
        ...
        if (match && hungarian.includes(`${match[1]} `))
        {
    ...
    function _fnLanguageCompat(oLanguage)
    {
      ...
      const zeroRecords = oLanguage.sZeroRecords
    ...
    {
      const out = []
      for (let i = 0, iLen = aiColumns.length; i < iLen; i++)
      {
    ...
    function _fnGetCellData(oSettings, iRow, iCol, sSpecific)
    {
      ...
      let sData = oCol.fnGetData(oData, sSpecific)
    ...
    function _fnGetObjectDataFn(mSource)
    {
      ...
      else if (typeof mSource === 'string' && (mSource.includes('.')
        || mSource.includes('[') || mSource.includes('(')))
      {
    ...
    function _fnGetRowElements(settings, row)
    {
      ...
      const attr = function (str, data, td) {
        if (typeof str === 'string') {
          const idx = str.indexOf('@');

          if (idx !== -1) {
            const src = str.substring(idx + 1)
     ...
     *  @param {boolean} [bIncludeHidden] If true then include the hidden columns in the calc,
     *  @memberof DataTable#oApi
     */
    ...
    function _fnAjaxUpdateDraw (settings, json)
    {
      ...
      const rocordsFiltered = compat('iTotalDisplayRecords', 'recordsFiltered');
      ...
      settings._iRecordsDisplay = Number.parseInt(rocordsFiltered, 10);
    ...
    function _fnFeatureHtmlFilter (settings)
    {
      ...
      let str = settings.oLanguage.sSearch;
      ...
      const jqFilter = $('input[type="search"]', filter)
    ...
    {
      const afnFilters = DataTable.ext.search
      const aiFilterColumns = _fnGetColumns(oSettings, 'bSearchable');

      for (let i = 0, iLen = afnFilters.length; i < iLen; i++)
      {
        let iCorrector = 0;
        for (let j = 0, jLen = oSettings.aiDisplay.length; j < jLen; j++)
        {
    ...
    {
      let asSearch
      let sRegExpString = bRegex ? sSearch : _fnEscapeRegex(sSearch)
      ...
      for (i = 0, ien = settings.aoData.length; i < ien; i++) {
        ...
        if (!row._aFilterData) {
          ...
          for (j = 0, jen = columns.length; j < jen; j++) {
            ...
            if (cellData.indexOf && cellData.includes('&')) {
    ...
    function _fnFeatureHtmlLength (settings)
    {
      ...
      const a = settings.oLanguage.sLengthMenu.split(/(_MENU_)/)
      div.children()
        .append(a[0])
        .append(select)
        .append(a[2])
    ...
    function _fnCalculateColumnWidths (oSettings)
    {
      ...
      else
      {
        ...
        const tmpTable = $(table.cloneNode(false))
        ...
        if (scrollX)
        {
          ...
          for (i = 0; i < visibleColumns.length; i++) {
            ...
            total += column.sWidthOrig === null
              ? outerWidth
              : Number.parseInt(column.sWidth, 10) + outerWidth - $(headerCells[i]).width();
          }
    ...
    function _fnSortAria (settings)
    {
      ...
      for (let i = 0, iLen = columns.length; i < iLen; i++)
      {
        ...
        const jqTh = $(col.nTh).removeAttr('aria-sort')
     ...
     *  @param {boolean} [append] Append the requested sort to the existing
    ...
    function _fnLoadState (oSettings, oInit)
    {
      ...
      const oData = oSettings.fnStateLoadCallback.call(oSettings.oInstance, oSettings);
      if (!oData)
      {
        return
      }
      ...
      const savedSort = oData.aaSorting
    ...
    DataTable = function (options)
    {
       ...
       *  @param {string} [oOpts.filter] Select TR elements that meet the current filter
       *    criterion ("applied") or all TR elements (i.e. no filter).
       *  @param {string} [oOpts.order] Order of the TR elements in the processed array.
       *    Can be either 'current', whereby the current sorting of the table is used, or
       *    'original' whereby the original order the data was read into the table is used.
       *  @param {string} [oOpts.page] Limit the selection to the currently displayed page
       ...
       *  @param {string|node|jQuery} sSelector jQuery selector or node collection to act on
       *  @param {object} [oOpts] Optional parameters for modifying the rows to be included
       *  @param {string} [oOpts.filter] Select elements that meet the current filter
       *    criterion ("applied") or all elements (i.e. no filter).
       *  @param {string} [oOpts.order] Order of the data in the processed array.
       *    Can be either 'current', whereby the current sorting of the table is used, or
       *    'original' whereby the original order the data was read into the table is used.
       *  @param {string} [oOpts.page] Limit the selection to the currently displayed page
       *    ("current") or not ("all"). If 'current' is given, then order is assumed to be
       ...
       * the Api's context.
       * @param {boolean} [traditional] Set the API instance's context to be
       ...
       *    </ul>
       *  @param {bool} [redraw] redraw the table or not
       ...
       *  @param {boolean} [bRedraw] Redraw the table or not, you will typically want to
       *  @dtopt API
       *  @deprecated Since v1.10
       *
       *  @example
       ...
       *  @param {bool} [bRedraw] redraw the table or not
       *  @dtopt API
       *  @deprecated Since v1.10
       *
       *  @example
       ...
       *  @param {boolean} [remove] Completely remove the table from the DOM
       *  @dtopt API
       *  @deprecated Since v1.10
       *
       *  @example
      ...
      /**
       * Redraw the table
       *  @param {bool} [complete] Re-filter and resort (if enabled) the table before the draw.
       *  @dtopt API
       *  @deprecated Since v1.10
       *
       *  @example
       ...
       *  @param {bool} [bRegex] Treat as regular expression or not
       *  @param {bool} [bSmart] Perform smart filtering or not
       *  @param {bool} [bShowGlobal] Show the input global filter in it's input box(es)
       *  @param {bool} [bCaseInsensitive] Do case-insensitive matching (true) or not (false)
       *  @dtopt API
       *  @deprecated Since v1.10
       *
       *  @example
       ...
       *    update a whole row.
       *  @param {bool} [bRedraw] Redraw the table or not
       *  @param {bool} [bAction] Perform pre-draw actions or not
      ...
      this.each(function () {
        ...
        if ($.inArray(true, $.map(stripeClasses, (el, i) => {
    ...
    const _toSettings = function (mixed)
    {
      let idx, jq
      const settings = DataTable.settings;
      const tables = $.map(settings, (el, i) => {
        return el.nTable;
      })
    ...
    _Api.extend = function (scope, obj, ext)
    {
      ...
      const methodScoping = function (fn, struc) {
        return function () {
    ...
    const _selector_run = function (selector, select)
    {
      const
        out = []; let res;
    ...
    {
      return _selector_run(selector, (sel) => {
    ...
    _api_register('rows()', function (selector, opts) {
      ...
      const inst = this.iterator('table', (settings) => {
    ...
    _api_register('rows().data()', function () {
      return this.iterator(true, 'rows', (settings, rows) => {
    ...
    const __details_display = function (show) {
      const ctx = this.context;
      ...
      if (_pluck(settings.aoData, '_details').length > 0) {
        ...
        table.on('draw.DT_details', () => {
          table.find('tbody tr').each(function () {
            ...
            const rowIdx = _fnNodeToDataIndex(settings, this);
            const row = settings.aoData[rowIdx];
        ...
        table.on('column-visibility.DT_details', (e, settings, idx, vis) => {
          ...
          // a colspan)
          let row; const visible = _fnVisbleColumns(settings);
    ...
    const __column_selector = function (settings, selector, opts)
    {
      ...
      return _selector_run(selector, (s) => {
        ...
        if (s === '') {
          ...
          const match = s.match(__re_column_selector)
            ...
            switch (match[2]) {
              ...
              case 'visible':
                ...
                if (idx < 0) {
                  ...
                  const visColumns = $.map(columns, (col, i) => {
    ...
    const __cell_selector = function (settings, selector, opts)
    {
      const data = settings.aoData;
      const rows = _selector_row_indexes(settings, opts)
      const cells = _pluck_order(data, rows, 'anCells')
     ...
     *
     *  @param {boolean} [visible] Flag to indicate if you want all (default)
    ...
    {
      return jQuery.map(DataTable.settings, (o) => {
    ...
    _api_register('destroy()', function (remove) {
      ...
      return this.iterator('table', (settings) => {
        ...
        const rows = $.map(settings.aoData, (r) => { return r.nTr });
        let i, ien;
    ...
    const __numericReplace = function (d, re1, re2) {
      if (!d || d === '-') {
        return -Infinity
      }
    ...
    $.extend(DataTable.ext.type.detect, [
      ...
      function (d)
      {
        return _empty(d) || (typeof d === 'string' && d.includes('<'))
          ? 'html'
          : null;
      }
    ])
