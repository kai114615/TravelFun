...
(function (api) {
  ...
  // Globals
  let fullscreen; let config; const callbacks = {
    youtube: []
  };
  ...
  function _browserSniff() {
    ...
    // MSIE 11
    if ((navigator.appVersion.includes('Windows NT')) && (navigator.appVersion.includes('rv:11'))) {
      name = 'IE';
      fullVersion = '11;';
    }
    // MSIE
  ...
  function _inArray(haystack, needle) {
    return Array.prototype.indexOf && (haystack.includes(needle))
  }
  ...
  function _toggleClass(element, name, state) {
    if (element) {
      if (element.classList) {
        ...
        const className = (` ${element.className} `).replace(/\s+/g, ' ').replace(` ${name} `, '')
  ...
  function _triggerEvent(element, event) {
    // Create faux event
    const fauxEvent = document.createEvent('MouseEvents')
  ...
  function Plyr(container) {
    ...
    function _seekManualCaptions(time) {
      ...
      if (player.media.currentTime.toFixed(1) >= _timecodeMin(player.captions[player.subcount][0])
        ...
        // Trim caption text
        const content = player.currentCaption.trim()
    ...
    function _injectControls() {
      ...
      // Setup tooltips
      if (config.tooltips) {
        const labels = _getElements(config.selectors.labels)
    ...
    function _setupPlayAria() {
      ...
      let label = player.buttons.play.innerText || config.i18n.play;
    ...
    function _setupYouTube(id) {
      ...
      const containers = _getElements('[id^="youtube"]')
      ...
      if (typeof YT === 'object') {
        ...
        callbacks.youtube.push(() => {
          _YTReady(id, container);
        })

        // Setup callback for the API
        window.onYouTubeIframeAPIReady = function () {
          for (let i = callbacks.youtube.length - 1; i >= 0; i--) {
            // Fire callback
            callbacks.youtube[i]();
      ...
      player.embed = new YT.Player(container.id, {
        ...
        events: {
          onReady(event) {
            ...
            // Setup buffering
            player.timer.buffering = window.setInterval(() => {
          ...
          onStateChange(event) {
            ...
            switch (event.data) {
              ...
              case 1:
                ...
                player.timer.playing = window.setInterval(() => {
    ...
    function _setupCaptions() {
      if (player.type === 'video') {
        ...
        let captionSrc = ''
        let kind
        const children = player.media.childNodes
        ...
        // If caption file exists, process captions
          ...
          let tracks = player.media.textTracks;
    ...
    function _setupFullscreen() {
      if (player.type != 'audio' && config.fullscreen.enabled) {
        // Check for native support
        const nativeSupport = fullscreen.supportsFullScreen
    ...
    function _seek(input) {
      let targetTime = 0
      const paused = player.media.paused
    ...
    function _toggleFullscreen(event) {
      ...
      let hoverTimer; let isMouseOver = false;
    ...
    function _setVolume(volume) {
      ...
      player.media.volume = Number.parseFloat(volume / 10);
    ...
    function _updateVolume() {
      ...
      const volume = player.media.muted ? 0 : (player.media.volume * 10)
    ...
    function _toggleCaptions(show) {
      ...
      if (typeof show !== 'boolean') {
        show = (!player.container.className.includes(config.classes.captions.active))
      }
    ...
    function _checkLoading(event) {
      ...
      player.loadingTimer = setTimeout(() => {
    ...
    function _updateProgress(event) {
      let progress = player.progress.played.bar
      let text = player.progress.played.text
      let value = 0;
    ...
    function _updateTimeDisplay(time, element) {
      ...
      player.secs = Number.parseInt(time % 60);
      player.mins = Number.parseInt((time / 60) % 60);
      player.hours = Number.parseInt(((time / 60) / 60) % 60);

      // Do we need to display hours?
      const displayHours = (Number.parseInt(((player.media.duration / 60) / 60) % 60) > 0)
    ...
    function _displayDuration() {
      const duration = player.media.duration || 0
    ...
    function _listeners() {
      ...
      const inputEvent = (player.browser.name == 'IE' ? 'change' : 'input')
      ...
      function checkFocus() {
        ...
        for (const button in player.buttons) {
          const element = player.buttons[button]

          _toggleClass(element, 'tab-focus', (element === focused));
        }
      }
      _on(window, 'keyup', (event) => {
      ...
      // Pause
      _on(player.buttons.pause, 'click', () => {
        _pause();
        setTimeout(() => {
      ...
      if (player.type === 'video' && config.click) {
        _on(player.videoContainer, 'click', () => {
    ...
    function _destroy() {
      ...
      const clone = player.media.cloneNode(true)
  ...
  api.supported = function (type) {
    const browser = _browserSniff();
    const oldIE = (browser.name === 'IE' && browser.version <= 9);
    const iPhone = /iPhone|iPod/i.test(navigator.userAgent);
    const audio = !!document.createElement('audio').canPlayType;
    const video = !!document.createElement('video').canPlayType;
    let basic; let full;
