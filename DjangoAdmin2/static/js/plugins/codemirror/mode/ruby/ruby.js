...
(function (mod) {
  ...
  CodeMirror.defineMode('ruby', (config) => {
    ...
    const indentWords = wordObj(['def', 'class', 'case', 'for', 'while', 'module', 'then', 'catch', 'loop', 'proc', 'begin'])
    ...
    function tokenBase(stream, state) {
      ...
      if (ch == '`' || ch == '\'' || ch == '"') {
        ...
        let style = 'string'; let embed = true;
        ...
        stream.skipToEnd();
        return 'comment'
      } else if (ch == '<' && (m = stream.match(/^<-?[`"']?([a-z_?]\w*)[`"']?(?:;|$)/i))) {
        return chain(readHereDoc(m[1]), stream, state);
      } else if (ch == '0') {
        if (stream.eat('x')) stream.eatWhile(/[\da-f]/i)
        else if (stream.eat('b')) stream.eatWhile(/[01]/);
        else stream.eatWhile(/[0-7]/);
        return 'number'
      } else if (/\d/.test(ch)) {
        stream.match(/^[\d_]*(?:\.[\d_]+)?(?:e[+\-]?[\d_]+)?/i)
        return 'number'
      } else if (ch == '?') {
        ...
        return 'operator'
      } else if (ch == '@' && stream.match(/^@?[a-z_\xA1-\uFFFF]/i)) {
        stream.eat('@')
        stream.eatWhile(/[\w\xA1-\uFFFF]/)
        return 'variable-2'
      } else if (ch == '$') {
    ...
    return {
      ...
      token (stream, state) {
        ...
        let style = state.tokenize[state.tokenize.length - 1](stream, state); let kwtype
        let thisTok = curPunc
        if (style == 'ident') {
