...
(function (mod) {
  ...
  CodeMirror.defineMode('q', (config) => {
    ...
    function tokenBase(stream, state) {
      const sol = stream.sol(); let c = stream.next();
      curPunc = null;
      if (sol) {
        if (c == '/') {
      ...
      if (c == '`')
        return stream.eatWhile(/[\w|:/.]/), 'symbol';
      if ((c == '.' && /\d/.test(stream.peek())) || /\d/.test(c)) {
        ...
        else if (stream.match(/^0[NwW]/)
          || stream.match(/^0x[\d|a-fA-F]*/)
          || stream.match(/^[0|1]+b/)
          || stream.match(/^\d+[chijn]/)
      ...
      if (/[A-Z|.]/i.test(c))
        return stream.eatWhile(/[\w|.]/), keywords.test(stream.current()) ? 'keyword' : 'variable';
      if (/[|/&^!+:\\\-*%$=~#;@><.,?_']/.test(c))
        return null;
      if (/[{}([\])]/.test(c))
        return null;
