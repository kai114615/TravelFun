...
(function (mod) {
  ...
  CodeMirror.defineMode('sql', (config, parserConfig) => {
    ...
    function tokenBase(stream, state) {
      ...
      if (support.hexNumber == true
        && ((ch == '0' && stream.match(/^x[0-9a-f]+/i))
          || (ch == 'x' || ch == 'X') && stream.match(/^'[0-9a-f]+'/i))) {
      // hex
      ...
      // .1 for 0.1
        ...
        if (support.ODBCdotTable == true && stream.match(/^[a-z_]+/i)) {
          return 'variable-2'
        }
      ...
      // operators
        ...
        && (stream.match(/^( )*([dDtT]|ts|TS)( )*'[^']*'( )*\}/) || stream.match(/^( )*([dDtT]|ts|TS)( )*"[^"]*"( )*\}/))) {
  ...
  (function () {
    ...
    // ref: http://dev.mysql.com/doc/refman/5.5/en/null-values.html
      ...
      return stream.match(/^[a-z.#!?]/i) ? 'variable-2' : null;
    }
